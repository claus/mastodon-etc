import type { NodeInfo, NodeInfoSplit, SplitOptions } from './types';
...
export function split(elSource: HTMLElement, options: Required<SplitOptions>): SplitResult {
    const { debug } = options;
    ...
    const { totalChars, totalLines } = cleanUp(elSource, elSplit, blockBucketsMeasured, options);
    ...
    return { charWrappers, totalChars, totalLines };
}
...
export function cleanUp(
    ...
    const result: CleanUpResult = { totalChars: 0 };
        ...
        .forEach(span => {
            ...
            if (isChar || isWhitelisted) {
                ...
                if (!isSpace) {
                    ...
                    // Index this span.
                    span.style.setProperty('--char-index', result.totalChars.toString());
                    result.totalChars++;
                }
            }
        });
    ...
    elSplit.style.setProperty('--total-chars', result.totalChars.toString());
    ...
    if (splitLines) {
        ...
        elSplit.querySelectorAll<HTMLElement>('[data-typeinternal="line"]').forEach(line => {
            ...
            // Index this line.
            line.style.setProperty('--line-index', result.totalLines!.toString());
            result.totalLines!++;
        });

        elSplit.style.setProperty('--total-lines', result.totalLines!.toString());
    }
